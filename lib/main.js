...
exports.main = async function({
        ...
        const resInputFile = path.resolve(inputFile);
        const resSrtList = srtFileList ? srtFileList.map(srtFile => path.resolve(srtFile)) : [];
        ...
        const jobAndCards = timesAndTexts.map((time, indx) => {            
            const outputFileName = path.join(resOutput, `${padNumber(indx + 1, fileNameLen)}.${suffix}`);
